...
def find_best_bin(required_quantity, warehouse_id=None, zone_id=None):
    ...
    Args:
        required_quantity: How many units need to be stored
        warehouse_id: Optional - filter by warehouse
        zone_id: Optional - filter by zone
    
    Returns:
        dict with recommended bin and alternatives, or None
    """
    ...
    for bin_obj in bins:
        ...
        if available >= required_quantity:
            utilization_after = ((bin_obj.capacity - available + required_quantity) / bin_obj.capacity) * 100
            candidates.append({
                'bin': bin_obj,
    ...
    # Best Fit: sort by least remaining space after placement (tightest fit)
